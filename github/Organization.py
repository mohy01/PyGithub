...
class Organization(CompletableGithubObject):
    ...
    def add_repository_to_organization_secret(
        ...
        status, headers, data = self._requester.requestJson(
            "PUT",
            f"{self.url}/actions/secrets/{secret_name}/repositories/{repository.id}",
        )
        return status == 204

    def add_repository_to_organization_dependabot_secret(
        self,
        secret_name,
        repository,
    ):
        """
        :calls: `PUT /orgs/{org}/dependabot/secrets/{secret_name}/repositories/{repository_id} <https://docs.github.com/en/rest/dependabot/secrets#add-selected-repository-to-an-organization-secret>`_
        :param secret_name: string
        :param repository:  :class:`github.Repository.Repository`
        :rtype: bool
        """
        ...
        status, headers, data = self._requester.requestJson(
            "PUT",
            f"{self.url}/dependabot/secrets/{secret_name}/repositories/{repository.id}",
        )
        return status == 204
